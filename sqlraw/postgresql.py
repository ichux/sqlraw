...
from sqlraw.psql_support import (
    DSN,
    IS_MIGRATION_TABLE,
    PGSQL_DOWN,
    PGSQL_MIGRATION_DOWN,
    PGSQL_MIGRATION_UP,
    PGSQL_UP,
    REVISION_EXISTS,
)
...
def db_initialise():
    ...
    """
    generate_migration_file()
    if not PostgresScheme.fetch_one(
        IS_MIGRATION_TABLE, **{"args": {"schema": SCHEMA}}
    ).exists:
        with open(MIGRATION_FILE, "r") as init_sql:
            ...
            if f"CREATE TABLE IF NOT EXISTS {SCHEMA}.{MIGRATION_TABLE}" not in data:
                ...
                with open(sql_file, "w") as save_sql:
                    up = PGSQL_MIGRATION_UP.format(
                        f"upgrade-{when}", when, MIGRATION_TABLE, SCHEMA
                    )
                    down = PGSQL_MIGRATION_DOWN.format(
                        f"downgrade-{when}", MIGRATION_TABLE, SCHEMA
                    )
...
def db_upgrade():
    ...
    for time_step in [_.strip(".sql") for _ in migration_files()]:
        decide = PostgresScheme.fetch_one(
...
def db_downgrade(step):
    ...
    try:
        ...
        for _ in to_use:
            count += 1
            if PostgresScheme.fetch_one(
                REVISION_EXISTS, **{"args": {"revision": _}}
            ).exists:
...
def status():
    ...
    try:
        for step in to_use:
            if PostgresScheme.fetch_one(
                REVISION_EXISTS, **{"args": {"revision": step}}
            ).exists:
